...
STATE_META = {
    ...
    "potentially_informative": {
        "label": "POTENCIALMENTE INFORMATIVA",
        "background": "#fef3c7",
        "border": "#d97706",
        "text": "#92400e",
    },
...
def user_state_from_decision(decision: str) -> str:
    if decision == "capture":
        return "informative"
    if decision in {"liver_without_reference", "doubtful"}:
        return "potentially_informative"
    if decision in {"warming_up", "analyzing", ""}:
        return "analyzing"
    return "not_informative"


def guidance_message(decision: str, fallback: str = "") -> str:
    messages = {
        "capture": (
        ...
        "liver_without_reference": (
            "Imagen potencialmente informativa. El hígado es visible, pero la "
            "referencia anecoica aún no cumple los criterios; mantenga la ventana "
            "anatómica y ajuste suavemente la inclinación de la sonda."
        ),
        "adjust": (
        ...
        "doubtful": (
            "Imagen potencialmente informativa. Mantenga la ventana y realice un "
            "ajuste fino de posición o inclinación para confirmar la vista."
        ),
...
def render_status_indicator(decision: str, message: str, target=None) -> None:
    ...
    render_status_state(
        state,
        guidance_message(decision, message),
        target,
    )
...
def render_completion_notice(target=None) -> None:
    ...
    renderer.markdown(
        ...
        <div class="completion-banner">
            ...
            Revise la retroalimentación final y descargue las imágenes informativas confirmadas.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_compact_summary(
    ...
    potentially_informative = int((states == "potentially_informative").sum())
    if informative > 0:
        final_state = "informative"
        final_message = "Se confirmaron imágenes informativas durante el análisis."
    elif potentially_informative > 0:
        final_state = "potentially_informative"
        final_message = (
            "No se confirmó una captura, pero se observaron imágenes potencialmente "
            "informativas que requieren un ajuste fino de la sonda."
        )
    else:
        final_state = "not_informative"
        final_message = "No se confirmaron imágenes informativas durante el análisis."
...
def show_longitudinal_guidance(decision: str, message: str) -> None:
    if decision == "capture":
        st.success(guidance_message(decision, message))
    elif decision == "liver_without_reference":
        st.warning(guidance_message(decision, message))
    else:
        st.error(guidance_message(decision, message))


def show_dino_guidance(decision: str, message: str) -> None:
    if decision == "capture":
        st.success(guidance_message(decision, message))
    elif decision == "doubtful":
        st.warning(guidance_message(decision, message))
    elif decision == "warming_up":
        st.info(guidance_message(decision, message))
    else:
        st.error(guidance_message(decision, message))
...
st.caption(
    "Seleccione la vista, cargue un video y siga la retroalimentación "
    "verde, amarilla o roja durante el análisis."
)
...
if view == "longitudinal":
    ...
    st.caption(
        "DINOv2-Small analiza una ventana de frames; el clasificador de la "
        "vista genera retroalimentación verde, amarilla o roja."
    )
