...
def find_checkpoint(class_name: str) -> Path:
    """Carga el alias congelado del manifiesto final seleccionado solo con validacion."""
    final_models = REPORTS_ROOT.parent / "final_models"
    selection_path = final_models / "selected_models_manifest.csv"
    if not selection_path.exists():
        raise FileNotFoundError(
            "Falta final_models/selected_models_manifest.csv. "
            "Ejecute scripts/09_export_selected_models.py o indique los tres checkpoints."
        )
    selection = pd.read_csv(selection_path)
    rows = selection[selection["class_name"] == class_name]
    if rows.empty:
        raise FileNotFoundError(f"No existe seleccion final para {class_name}.")
    checkpoint_path = Path(str(rows.iloc[0]["alias_checkpoint"]))
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Checkpoint final no encontrado: {checkpoint_path}")
